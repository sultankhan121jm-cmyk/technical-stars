...
const Services = () => {
  const { t, lang } = useLang(); // <-- THIS IS THE FIX (lang was missing in my last version)
  return (
    <main className="pt-[66px] bg-brand-bg">
      ...
      <section className="py-20 md:py-28 bg-white">
        <div className="max-w-7xl mx-auto px-5 lg:px-16 text-center">
          <motion.nav initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-brand-muted text-[13px]">
            <Link to="/" className="hover:text-brand-blue transition-colors">{t("about.breadcrumbHome")}</Link> <span className="mx-2">›</span> <span className="text-brand-navy font-medium">{t("nav.services")}</span>
          </motion.nav>
          <motion.h1 initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="mt-3 text-3xl md:text-4xl lg:text-5xl font-bold text-brand-navy">{t("nav.services")}</motion.h1>
          <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.2 }} className="mt-3 text-brand-muted max-w-2xl mx-auto">{lang === "ar" ? "خدمات صيانة وإصلاح احترافية في جميع أحياء الرياض" : "Professional home repair and maintenance across all Riyadh districts"}</motion.p>
          <div className="section-line mx-auto mt-4" />
          <img src={servicesBanner} alt="Services" loading="lazy" className="mt-10 max-w-4xl mx-auto w-full rounded-2xl object-cover max-h-[280px] md:max-h-[360px] shadow-lg shadow-brand-navy/10" />
        </div>
      </section>
      ...
      <section className="py-20 md:py-28 bg-brand-bg">
        <div className="max-w-7xl mx-auto px-5 lg:px-16">
          <div className="text-center">
            ...
            <h2 className="mt-3 text-3xl md:text-4xl font-bold text-brand-navy">{lang === "ar" ? "كل ما يحتاجه منزلك" : "Everything Your Home Needs"}</h2>
            <div className="section-line mx-auto mt-4" />
          </div>
      ...
      {/* Districts Section - White background */}
      <section className="py-16 md:py-20 bg-white">
        <div className="max-w-7xl mx-auto px-5 lg:px-16">
          <div className="text-center">
            <span className="text-brand-blue text-[12px] font-semibold tracking-widest uppercase">{lang === "ar" ? "التغطية" : "Coverage"}</span>
            <h2 className="mt-3 text-2xl md:text-3xl font-bold text-brand-navy">{lang === "ar" ? "نغطي جميع أحياء الرياض" : "We Cover All Riyadh Districts"}</h2>
            <div className="section-line mx-auto mt-4" />
          </div>
          <div className="mt-10 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3">
            {districts.map((d, i) => (
              <motion.div key={i} initial={{ opacity: 0, y: 10 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.03 }} className="bg-brand-light rounded-xl text-brand-navy text-[13px] font-medium text-center py-3 px-3 hover:bg-brand-blue hover:text-white transition-all duration-200 min-h-[44px] flex items-center justify-center">
                {/* THE FIX IS HERE: Parsing the object correctly */}
