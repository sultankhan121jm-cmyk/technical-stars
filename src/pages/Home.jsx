...
import { FaClock, FaShieldAlt, FaUserCheck, FaThumbsUp, FaPhoneAlt, FaSearch, FaTools, FaCheckCircle, FaChevronDown } from "react-icons/fa";
...
const SectionHead = ({ label, title, subtitle }) => (
  <div className="text-center">
    ...
    <motion.h2 initial={{ opacity: 0, y: 15 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.1 }} className="mt-3 text-3xl md:text-4xl font-bold text-brand-navy">{title}</motion.h2>
    ...
    {subtitle && <motion.p initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} transition={{ delay: 0.2 }} className="mt-4 text-brand-muted max-w-2xl mx-auto">{subtitle}</motion.p>}
  </div>
);
  ...
  return (
    <section className="py-20 md:py-28 bg-white">
      <div className="max-w-7xl mx-auto px-5 lg:px-16">
        ...
        {/* CHANGED: grid-cols-2 for mobile, tighter gap-3 */}
        <div className="mt-14 grid grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-5 lg:gap-6">
        ...
        <motion.div initial={{ opacity: 0, y: 15 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="mt-12 text-center">
          <p className="text-sm text-brand-muted">{t("servicesSection.needService")}</p>
    ...
    <section className="py-20 md:py-28 bg-brand-light">
      <div className="max-w-7xl mx-auto px-5 lg:px-16 grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
        <div className="text-center lg:text-left">
          ...
          <motion.p initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} transition={{ delay: 0.2 }} className="mt-5 text-brand-muted max-w-xl mx-auto lg:mx-0">{t("whyChoose.subtitle")}</motion.p>
        </div>

        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {featureKeys.map((key, i) => {
            const Icon = icons[i]; return (
              <motion.div key={key} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }} className="bg-white rounded-xl p-6 border border-brand-border">
                <Icon className="text-brand-blue text-xl" />
                <h3 className="mt-3 text-brand-navy text-[15px] font-semibold">{t(`whyChoose.features.${key}.title`)}</h3>
                <p className="mt-1.5 text-brand-muted text-sm">{t(`whyChoose.features.${key}.desc`)}</p>
              </motion.div>
            );
          })}
        </div>
          ...
          {stepKeys.map((key, i) => {
            const Icon = icons[i]; return (
              <motion.div key={key} initial={{ opacity: 0, y: 25 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.12 }} className="flex flex-col items-center text-center">
                ...
                <h3 className="mt-4 text-brand-navy text-[15px] font-semibold">{t(`howItWorks.steps.${key}.title`)}</h3>
                <p className="mt-2 text-brand-muted text-sm max-w-[240px]">{t(`howItWorks.steps.${key}.desc`)}</p>
              </motion.div>
            );
          })}
        </div>
    ...
    <section className="py-20 md:py-28 bg-white">
      <div className="max-w-3xl mx-auto px-5 lg:px-16">
        ...
        <motion.div initial={{ opacity: 0, y: 15 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="mt-12 space-y-3">
          {faqs.map((faq, i) => (
            <div key={faq.id} className={`bg-brand-light border rounded-xl transition-colors ${openIndex === i ? "border-brand-blue/20 bg-brand-blue/[0.02]" : "border-brand-border"}`}>
              <button onClick={() => setOpenIndex(openIndex === i ? null : i)} className="w-full flex items-center justify-between p-5 text-left">
                <span className="text-brand-navy text-sm font-medium pr-4">{getQ(faq)}</span>
                <motion.span animate={{ rotate: openIndex === i ? 180 : 0 }} className="text-brand-blue text-xs flex-shrink-0"><FaChevronDown /></motion.span>
              </button>
              <AnimatePresence initial={false}>
                {openIndex === i && (
                  <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.3 }} className="overflow-hidden">
                    <p className="px-5 pb-5 text-brand-muted text-sm leading-relaxed border-t border-brand-border pt-4">{getA(faq)}</p>
                  </motion.div>
                )}
              </AnimatePresence>
            </div>
